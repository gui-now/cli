...
async function createCanvas(content: string, opts: { title?: string; expires?: string; markdown: boolean }): Promise<ApiResponse> {
  ...
  if (opts.command === 'push') {
    ...
    if (opts.file) {
      ...
      try {
        ...
        console.error(red(`Error: Cannot read stdin — ${(err as Error).message}`))
        process.exit(1)
      }
    }
